...
export function Header() {
	...
	return (
		<header
			className={cn(
				'fixed top-4 left-1/2 -translate-x-1/2 z-[100] mx-auto w-[95%] max-w-3xl rounded-full border border-[#06b6d4]/50 bg-[#06b6d4]/10 backdrop-blur-xl shadow-[0_8px_32px_0_rgba(6,182,212,0.15)] transition-all ease-out',
				{
					'bg-black/95 border-transparent': open,
				},
			)}
		>
			<nav
			...
			>
				...
				<div className="hidden items-center gap-1 md:flex">
					{links.map((link, i) => (
						<a 
							key={i} 
							className={cn(buttonVariants({ variant: 'ghost', size: 'sm' }), "text-neutral-300 hover:text-white px-3 text-xs")} 
							href={link.href}
							onClick={(e) => {
								e.preventDefault();
				...
				<div className="hidden items-center md:flex">
					<a 
						href="/Priyanshi_Modi.pdf" 
						target="_blank" 
						rel="noopener noreferrer" 
						className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), "h-8 px-5 text-xs font-medium border-[#06b6d4]/30 text-white hover:bg-[#06b6d4]/20")}
					>
						Resume
					</a>
				</div>
				...
				<Button size="icon" variant="outline" onClick={() => setOpen(!open)} className="md:hidden border-white/10 text-white hover:bg-white/10 ml-auto">
			...
			<div
				className={cn(
					'bg-black/95 fixed top-16 right-0 bottom-0 left-0 z-[100] flex flex-col overflow-hidden border-y border-white/10 md:hidden',
					open ? 'block' : 'hidden',
				)}
			>
				<div
				...
				>
						...
						{links.map((link) => (
							<a
								...
								className={cn(buttonVariants({
									variant: 'ghost',
									className: 'justify-start text-2xl h-auto py-4 text-neutral-300 hover:text-white',
								}))}
								href={link.href}
							>
								{link.label}
					...
					<div className="flex flex-col gap-4 mb-12">
						<a 
							...
							target="_blank" 
							rel="noopener noreferrer" 
							className={cn(buttonVariants({ variant: 'outline' }), "w-full text-xl h-14 border-[#06b6d4]/30 text-white hover:bg-[#06b6d4]/20 flex items-center justify-center")}
						>
							Resume
						</a>
					</div>
